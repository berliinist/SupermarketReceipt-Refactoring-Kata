...
class TestReceiptIntegration(unittest.TestCase):
    ...
    def _setup_and_add_items_to_receipt(self, nr_items):
        self._setup_list_kwargs_of_each_item(nr_items)
        for i in range(nr_items):
            self.receipt.add_item(**self.kwargs_list[i])

    def _setup_list_kwargs_of_each_discount(self, nr_discounts):
        self.prod_kwargs_list = [setup_product_kwargs() for _ in range(nr_discounts)]
        self.disc_kwargs_list = [
            {'product': mdl_objcts.ProductInfo(**cp.deepcopy(self.prod_kwargs_list[i])),
             'description': f'Some discount number {i+1}',
             'discount_amount': round(random.random() * -10, 2)} for i, _ in enumerate(range(nr_discounts))]

    def _setup_and_add_discounts_to_receipt(self, nr_discounts):
        self._setup_list_kwargs_of_each_discount(nr_discounts)

        discounts = [mdl_objcts.Discount(**self.disc_kwargs_list[i]) for i in range(nr_discounts)]
    ...
    def test_assert_number_of_items(self, nr_items):
        self._setup_and_add_items_to_receipt(nr_items)
    ...
    def test_assert_correct_number_of_discounts_after_added(self, nr_discounts):
        _ = self._setup_and_add_discounts_to_receipt(nr_discounts)
        self.assertEqual(len(self.receipt.discounts), nr_discounts)

    @parameterized.expand([(1,), (4,)])
    def test_assert_correct_order_of_discounts(self, nr_discounts):
        discounts = self._setup_and_add_discounts_to_receipt(nr_discounts)
        self.assertListEqual(self.receipt.discounts, discounts)

    @parameterized.expand([(1,), (3,)])
    def test_assert_names_of_discounts(self, nr_discounts):
        _ = self._setup_and_add_discounts_to_receipt(nr_discounts)
        self.assertListEqual([self.receipt.discounts[i].product.name for i in range(nr_discounts)],
                             [self.prod_kwargs_list[i]['name'] for i in range(nr_discounts)])
    ...
    def test_assert_descriptions_of_discounts(self, nr_discounts):
        ...
        self.assertListEqual([self.receipt.discounts[i].description for i in range(nr_discounts)],
                             [self.disc_kwargs_list[i]['description'] for i in range(nr_discounts)])

    @parameterized.expand([(1,), (2, )])
    def test_assert_discount_amounts(self, nr_discounts):
        _ = discounts = self._setup_and_add_discounts_to_receipt(nr_discounts)
        self.assertListEqual([self.receipt.discounts[i].discount_amount for i in range(nr_discounts)],
                             [self.disc_kwargs_list[i]['discount_amount'] for i in range(nr_discounts)])
    ...
    def test_assert_receipt_total_price_if_discounts_only(self, nr_discounts):
        _ = discounts = self._setup_and_add_discounts_to_receipt(nr_discounts)
        expected = round(sum([self.disc_kwargs_list[i]['discount_amount'] for i in range(nr_discounts)]), 2)
        self.assertAlmostEqual(self.receipt.total_price(), expected, places=2)
    ...
    def test_assert_receipt_total_price_of_items_and_discounts(self, nr_items, nr_discounts):
        ...
        total_discounts = round(sum([self.disc_kwargs_list[i]['discount_amount'] for i in range(nr_discounts)]), 2)
